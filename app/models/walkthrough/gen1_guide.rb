module Walkthrough
  ...
  module Gen1Guide
    ...
    def line(*dexes) = dexes.map { |d| { dex: d, name: NAMES.fetch(d) } }

    def mon(dex, lvl) = { dex: dex, name: NAMES.fetch(dex), lvl: lvl }

    def mon_key(dex) = NAMES.fetch(dex).downcase.gsub("♀", "f").gsub("♂", "m").gsub(/[^a-z]/, "")

    def base(slug) = "#{self::K}.locations.#{slug.tr('-', '_')}"
    ...
    def reconcile_encounters(loc)
      ...
      kept, seen = [], []
      loc.encounters.each do |card|
        next kept << card unless from_table?(card)

        found = table_row(tables, card)
        next if found.nil?

        kind, mon = found
        kept << refreshed(card, mon)
        seen << [ card.dex, kind ]
      end
    ...
    def table_row(tables, card)
      tables.each do |place|
        ...
        next unless same_method?(kind, card)

        mon = place.fetch("mons").find { |row| row["dex"] == card.dex }
        return [ kind, mon ] if mon
      end
      nil
    end
    ...
    def refreshed(card, mon)
      rate, level = restated(card.at_map, card.dex, card.how, mon)
    ...
    def label_for(loc, kind)
      borrowed = loc.encounters.find { |card| from_table?(card) && same_method?(kind, card) }
      borrowed&.how || WILD_METHODS.fetch(kind)
    end

    def level_span(mon)
      low, high = mon.fetch("min_level"), mon.fetch("max_level")
      low == high ? low.to_s : "#{low}\u2013#{high}"
    end

    def enc(slug, dex, how, rate, level, rarity, *chain, tip: false, from: false, unlock: nil,
    ...
    def pikachu_friendship
      b = "#{self::K}.pikachu_friendship"
    ...
    def badge_img(badge) = "walkthrough/yellow/badges/#{badge.downcase}.png"

    def badge_guide
      b = "#{self::K}.badge_guide"
    ...
    def mew_tldr
      ...
      (1..5).map { |n| MewTldr.new(n, "#{mew_glitch_key}.tldr.#{n}.title", "#{mew_glitch_key}.tldr.#{n}.text", tones[n - 1]) }
    end
    ...
    def mew_untouched
      [ [ "trigger", "JR. TRAINER♂", "pink", "RT 24", "route-24-trainer-5-20" ],
        ...
        b = "#{mew_glitch_key}.untouched.#{key}"
    ...
    def mew_sleepers
      { "012" => "sleep_powder", "035" => "sing", "039" => "sing",
        ...
        MewSleeper.new(dex, NAMES.fetch(dex), "#{mew_glitch_key}.moves.#{move}")
      end
    end
    ...
    def mew_phases = MEW_PHASE_DEFS.map { |p| mew_phase(p) }

    def mew_phase(phase)
      b = "#{mew_glitch_key}.phases.#{phase[:key]}"
      MewPhase.new("#{b}.label", "#{b}.title", "#{b}.meta", phase[:tone],
        phase[:steps].each_with_index.map { |step, i| mew_step(b, step, i + 1) })
    end

    def mew_step(phase_base, step, n)
    ...
    def mew_spares
      b = "#{mew_glitch_key}.spares.trainers"
    ...
    def mew_stage_kind(stage, level)
    ...
    def step_map(area, first, last = first)
    ...
    def note_trainer(trainer, notes)
    ...
    def fill_hall(loc, field, fresh, claimed)
      hall = loc.public_send(field)
      return loc if hall.nil?
    ...
    def roster
      @roster ||= JSON.parse(File.read(File.join(__dir__, "#{self::DATA_PREFIX}_trainers.json"))).freeze
    end

    def manifest
      @manifest ||= JSON.parse(File.read(File.join(__dir__, "#{self::DATA_PREFIX}_maps.json"))).freeze
    end

    def wild_tables
      @wild_tables ||= JSON.parse(File.read(File.join(__dir__, "#{self::DATA_PREFIX}_encounters.json")))
    ...
    def map_marker(data, key = data["key"])
    ...
    def npc_marker(data, width, height, key)
      gx, gy = data["grid"]
    ...
    def loc(slug, kind, name, order, title: nil, steps: 3, shots: [], hidden_items: {}, key_items: {}, pins: {}, encounters: [], trainers: [], trades: [], oak_queue: [], badge: nil, gym: nil, dojo: nil, gym_after: nil, gym_finale: false, trivia: nil, grind: nil, later: [], second_after: nil)
      b = base(slug)
      Location.new(
    ...
    def leader(name, reward, *team, battle: nil, opp: nil) = tr("LEADER", name, reward, *team, battle: battle, opp: opp)

    def rival(reward, *team, where: nil, battle: nil, opp: nil, note: nil, note_link: nil)
      tr("RIVAL", "Blue", reward, *team, sprite: "blue-gen1two", where: where, battle: battle,
        opp: opp, note: note, note_link: note_link)
    end

    def gym(slug, name, type, badge, tm, leader, puzzle: [], trainers: [], needs: nil)
      b = base(slug)
      Gym.new(
    ...
    def gstep(slug, n, scene: nil, quiz: nil)
    ...
    def other_dex(dex) = DOJO_PICKS.map(&:second).find { |other| other != dex }

    def dojo_pick(b, (side, dex, knows, learns), other, best)
    ...
    def step(base, n, items: [], hidden: [], shots: [], html: false, link: nil, pins: {},
    ...
    def mart_item(name, rec: false, rec_key: nil, desc_key: nil, tick: nil)
      facts = item_catalog.fetch(name)
      tm = facts["tm"]
    ...
    def celadon_stock(id) = place_facts.fetch(CELADON_FLOOR_CONSTS.fetch(id)).stock

    def celadon_dept_store
    ...
    def dept_floor(id, kind, counters: [], gift: nil, trades: [], note: false)
      b = base("celadon-city")
    ...
    def dept_counter(floor_id, title, names, rec: [])
      b = base("celadon-city")
    ...
    def prize_facts
      @prize_facts ||= JSON.parse(File.read(File.join(__dir__, "#{self::DATA_PREFIX}_places.json")))
        .fetch("prizes").freeze
    end
    ...
    def item(base, n, name, key, at: nil, tick: nil)
    ...
    def authored_tick(base, cat, key) = "#{slug_of(base)}/#{cat}-#{key.tr('_', '-')}"

    def hidden(base, n, name, key, scene, pin, at: nil)
    ...
    def later(slug, key, name, kind, need, scene, pin = nil)
      base = base(slug)
    ...
    def grind_spot(base, anchor:, after_map:, art:, note_icon:, encounters:, mons:)
    ...
    def grind_mon(base, encounters, dex, tone, level)
    ...
    def missable(base, anchor:, after_step:)
    ...
    def trivia_card(base, key, dex, tone, this_state, rt22_state)
    ...
    def shot(label, caption = nil) = Shot.new(image: nil, label: label, caption_key: caption)
  end
end
