...
### Arrow-tile mazes (`spinners.py`)

A floor whose script carries a `map_coord_movement` table is crossed by being pushed. Each entry
names an arrow tile and an RLE list of moves (`db PAD_<DIR>, <count>`) read **backwards** from its
`$FF`, and `map_coord_movement` takes **x, y** even though `dbmapcoord` writes the bytes y,x.
`arrow_tiles` reads the table, `slide_from` chains a ride through every arrow it lands on, and
`leg` / `route` solve the shortest way between stops over a graph of "walk to a neighbour" and
"step onto an arrow and go wherever it puts you".

`drawn_route` is what the build emits, as `route` on the map's manifest entry: legs of pixel
points for the app to hang an SVG on. A floor gets one only if it has arrow tiles **and** its
`paths.ROUTES` entry names two or more stops, so the line and the pin letters come from the same
place. Editing either is manifest drift; `manifest_diff.py` reports it as `rerouted`.
...
- `sprites`: each takes a `SPRITE_*` id (or a raw `gfx/sprites` basename), a `grid` cell, a
  ...
  `gfx/emotes/*` name, e.g. `"shock"` for the spotted `!`) drawn one cell above it. A sprite's
  `emote` is only for a trainer the game gives a sightline to (the engage distance in its trainer
  header, read by `sources.parse_trainer_sight`); one with no header, or a distance of zero, is
  talked into a fight and never flashes anything. `test_generators.py` enforces it.
- `player_emote`: the same bubble over the hero instead. That is where a scripted ambush puts it:
  every Jessie & James cutscene sets `wEmotionBubbleSpriteIndex` to 0, the player.
...
- `cut`: `[[gx,gy], ...]` cells whose cuttable tree the shot is set *after*. The game swaps the
  ...
  cannot be cut. Use it wherever the walkthrough has already told the player to Cut: the Fisher's
  fenced plot in Viridian, and Route 10's hidden Super Potion, which you can only face from the
  stump of the tree beside the tunnel mouth.
