...
- **Credit the projects the art comes from.** None of this artwork is ours: the trainer sprites are
  ...
  from PokeAPI, and every map and screenshot is generated from the pret/pokeyellow disassembly. The
  footer carries that on every page (`pages.home.footer.credits_html`) and the avatar picker names
  Showdown again where its sprites fill the screen (`account.avatar.credit_html`). URLs live in one
