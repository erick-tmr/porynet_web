...
class ProgressImportTest < ActiveSupport::TestCase
  ...
  test "a run longer than one write may hold still lands in full" do
    crowd = (0..Progress::Sync::MAX_KEYS).to_h { |n| [ "route-3/item-#{n}", true ] }
    landed = Progress::Import.call(users(:rival), guest("collected" => { "yellow" => crowd }))

    assert_equal crowd.size, landed["yellow"].marks
    assert_equal crowd.size,
      Progress::Snapshot.marks(SaveFile.find_by!(user: users(:rival), game_slug: "yellow")).size,
      "what it reports landed and what landed are the same number"
  end
